...
const appendNewBox = (title, subTitle, tableRef) => {
    ...
    $newBoxBtn.addEventListener('click', e => {
        ...
        const tbl = _js_perilous_perilous_js__WEBPACK_IMPORTED_MODULE_1__["default"].table(tableRef);
        const $p = $result.querySelector('p') || document.createElement('p');
        $p.textContent = '...';
        
        tbl
            .then(tblObj => tblObj.roll())
            .then(tblObj => _js_perilous_perilous_js__WEBPACK_IMPORTED_MODULE_1__["default"].resolveStringWithNames(tblObj.result))
            .then(str => {
                $p.textContent = str;
...
const genBoxHtml = (title, subTitle, tableRef) => {
    ...
    return mustache__WEBPACK_IMPORTED_MODULE_2__["render"](template, view);
}
